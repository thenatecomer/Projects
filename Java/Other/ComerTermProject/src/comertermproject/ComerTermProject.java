...
public class ComerTermProject {
    ...
    public String getPostFix(){
        PostFix = type(Interface.getEquation());  
        return PostFix;
    }
     ...
     public String type(String sin){
        ...
        for (; c<sin.length();c++){
            ...
            //Case 3: if start of operator not negative or subtraction
            else if((sin.charAt(c)=='+')||(sin.charAt(c)=='*')||(sin.charAt(c)=='/')||(sin.charAt(c)=='%')||(sin.charAt(c)=='-')){
                System.out.println("****************************"+sin.charAt(c));
                if(S.next==0){ System.out.println("ONE IN ==0");
                    S.push(sin.charAt(c));
                }else if((checkPrecedence(sin.charAt(c))<(checkPrecedence(S.peek())))){System.out.println("ONE IN <");
                    int counter = 0;
                ...
                else if(S.next>=1&&checkPrecedence(sin.charAt(c))==(checkPrecedence(S.peek()))&&S.next!=0){System.out.println("ONE IN ==PPPPPP");
                    String v = ""+S.pop();
                    q.enqueue(v);
                    S.push(sin.charAt(c));
                }
                else if(S.next>=1&&checkPrecedence(sin.charAt(c))>(checkPrecedence(S.peek()))){System.out.println("ONE IN >");
                    String e = "" + sin.charAt(c+1);
                    q.enqueue(e);
                    e = "" + sin.charAt(c);
                    q.enqueue(e);
                    c++;
...
//            }
        ...
        String j = "";
            for(int wq=0;wq<q.array.length;wq++){
                j+=q.array[wq];
                System.out.println("VALUES ARE EEEEEEEEEEEE:"+q.array[wq]);
            }
       return j; 
