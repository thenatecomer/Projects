...
def main():
    ...
    print "Running ./test-trans -M 4 -N 4 -s 1 -E 2 -b 3"
    p = subprocess.Popen("./test-trans -M 4 -N 4 -s 1 -E 2 -b 3 | grep TEST_TRANS_RESULTS", 
                         shell=True, stdout=subprocess.PIPE)
    stdout_data = p.communicate()[0]
    ...
    print "Running ./test-trans -M 128 -N 128 -s 5 -E 2 -b 3"
    p = subprocess.Popen("./test-trans -M 128 -N 128 -s 5 -E 2 -b 3 | grep TEST_TRANS_RESULTS", 
                         shell=True, stdout=subprocess.PIPE)
    stdout_data = p.communicate()[0]
    ...
    trans32_score = computeMissScore(miss32, 14, 18, maxscore['trans32']) * int(result32[0])
    trans64_score = computeMissScore(miss64, 8262, 16262, maxscore['trans64']) * int(result64[0])
    if (miss64 <=8198):
        additional_score = 4
    else:
        additional_score = 0
    total_score = csim_cscore[0] + trans32_score + trans64_score + additional_score
    ...
    if miss32 == 2**31-1 :
        misses = "invalid"
    print "%-25s%11.1f%13d%15s" % ("Matrix Shift 4x4:", trans32_score, 
                                  maxscore['trans32'], misses)
#
    misses = str(miss64)
    if miss64 == 2**31-1 :
        misses = "invalid"
    print "%-25s%11.1f%13d%15s" % ("Matrix Shift 128x128:", trans64_score+additional_score, 
                                  maxscore['trans64']+maxscore['additional'], misses)

    print "%25s%11.1f%13d" % ("Total points", total_score,
                             maxscore['csim'] + 
                             maxscore['trans32'] +
                maxscore['trans64']+maxscore['additional'])
