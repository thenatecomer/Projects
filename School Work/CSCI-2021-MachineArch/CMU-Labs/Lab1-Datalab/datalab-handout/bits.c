...
int oddBits(void) {
	int partial = 0xAA;
	partial = (partial << 8) + partial;
	partial = (partial << 16) + partial;
	return partial;
}
/* 
...
int ezThreeEights(int x) {
	// ((x<<1)+x) to multiply by 3, (x>>3) to divide by 8
	int y = ((x<<1)+x);
	int z = 7 & (y >>31);
	// z to account for negative
  return (y+z) >> 3;
}
/* 
...
int addOK(int x, int y) {
	int add = x + y;
  	...
  	int firstY = (y>>31);
	int firstAdd = (add>>31);
	return !((firstX ^ firstAdd) & ~(firstX ^ firstY));
}
/*
...
unsigned float_neg(unsigned uf) {
	...
	unsigned notanumber = 0x7FC00000;
	unsigned neg = 0x80000000;
	if((uf == notanumber)||(uf == infinity)){
		return uf;
	}
	return (uf ^ neg);
}
